  ...
  reducers: {
    addToCart(state, action) {
      ...
      if (existingIndex >= 0) {
        ...
        toast.info("Augmentation de la quantité de produits", {
    ...
    decreaseCart(state, action) {
      ...
      if (state.cartItems[itemIndex].cartQuantity > 1) {
        ...
        toast.info("Quantité de produit diminuée", {
          position: "bottom-left",
        });
        ...
        toast.error("Diminuer la quantitée du produit", {
          position: "bottom-left",
        });
      }
      localStorage.setItem("cartItems", JSON.stringify(state.cartItems));
    ...
    clearCart(state, action) {
      state.cartItems = [];
      localStorage.setItem("cartItems", JSON.stringify(state.cartItems));
      toast.error(" Panier vide", { position: "bottom-left" });
    },
  },
});
